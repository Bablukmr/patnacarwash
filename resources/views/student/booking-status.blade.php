        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6">
                            <h5><strong>Status:</strong> <span class="badge {{ $booking->workAssignment->status ? 'bg-success' : 'bg-warning' }}">{{ $booking->workAssignment->status ?? 'Pending Assignment' }}</span></h5>
                        ...
                        <div class="col-lg-6">
                            ...
                            <p><strong>Assigned Employee:</strong> {{ $booking->workAssignment->employee->name }}</p>
                            <p><strong>Latitude:</strong> {{ $booking->workAssignment->latitude }}</p>
                            <p><strong>Longitude:</strong> {{ $booking->workAssignment->longitude }}</p>
                            <p><strong>Last Update:</strong> {{ $booking->workAssignment->dailyUpdates->last()->created_at ?? 'No updates yet' }}</p>
                            @endif
                        </div>
                    </div>
...
@section('extraCss')
<!-- Add custom CSS if necessary for fine-tuning -->
<style>
    .card-header {
...
@section('extraJs')
<!-- Optionally include extra JS or custom scripts for enhanced interaction -->
<script>
    $(document).ready(function() {
        // Add any custom JS here if required, e.g., for dynamically loading images
    });
</script>
@endsection
